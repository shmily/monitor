...
extern struct list_head OutZoneList;
...
static int  Zone_Packet_Over_Flag = 1;
       ...
       void OutZone_Packet_Over_CallBack (int success);
...
void OutZoneMonitor( void )
{
	...
	while(1){
		...
		if(current_cond.active == 0x00){		                    // no active rule, try to find one.
			...
			Get_Active_Cond(&OutZoneList, &gps_info, &current_cond, 0);
...
static void Monitor_Packet_Update(void)
{
    ...
    elememt.Update_callback   = OutZone_Packet_Over_CallBack;
